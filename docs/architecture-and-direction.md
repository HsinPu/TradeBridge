...
0.2.0 部署更新：整站加入可設定的 `APP_BASE_PATH`（預設 `/tradebridge`）。Nginx 與前端使用相同前綴，後端透過 FastAPI `root_path` 對應內部 `/api/v1` 路由；舊 API 保留相容入口。外層代理保留完整路徑、公開 Host 與協定資訊。變更前綴需重建前端；操作與映射範例見根目錄 README。
