...
在「設定」同步完整 Binance 現貨目錄，再到「任務 → 預覽並啟動」確認市場數、空間與舊排程衝突。政策預設關閉；目錄同步不會下載 K 線。啟用後自動納入目前可交易及後續新上市的所有報價交易對，先建立近期資料，再從各市場最早可取得時間分段回補。已下架且不在目錄的舊市場不會自動探索。

自動收集固定只存 1m，可暫停歷史、暫停全部、排除市場及重試問題區段。「資料」預設由 1m 彙整成 3m 至月線，使用 UTC 邊界及精確小數；舊原生週期資料仍可切換查看。空白時間不造假 K 線，掃描進度與連續完整度分開顯示。

每頁最多處理 31 天來源分鐘，較長時間範圍用游標接續。官方歷史檔先驗 SHA256，沒有檔案時改用 REST；可在資料頁建立有範圍的重新匯入任務。詳見 [功能、API、容量與升級說明](docs/plans/minute-data-implementation.md)。
